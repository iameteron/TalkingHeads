...
print("--------------------------------3.1")
from external_visualization.load_textures import TEXTURES, get_texture_bundle, load_all_textures

print("--------------------------------3.2")


def _render_obs_view_pixels_impl(state, block_pixel_size, textures_root):

    textures = textures_root[block_pixel_size]
...
def _render_inventory_pixels_impl(state, block_pixel_size, textures_root):
    textures = textures_root[block_pixel_size]
...
def _render_full_map_base_impl(state, block_pixel_size, textures_root):
    textures = textures_root[block_pixel_size]
    block_textures = textures["block_textures"].astype(jnp.float32)
    grid = state.map
    map_h = grid.shape[0]
    map_w = grid.shape[1]
    h_px = map_h * block_pixel_size
    w_px = map_w * block_pixel_size

    block_ids = jnp.repeat(
        jnp.repeat(grid, repeats=block_pixel_size, axis=0),
        repeats=block_pixel_size,
        axis=1,
    )
    local_x = jnp.broadcast_to(
        (jnp.arange(h_px) % block_pixel_size)[:, None],
        (h_px, w_px),
    )
    local_y = jnp.broadcast_to(
        (jnp.arange(w_px) % block_pixel_size)[None, :],
        (h_px, w_px),
    )
    return block_textures[block_ids, local_x, local_y]


def _render_craftax_pixels_impl(state, block_pixel_size, textures_root):
    map_pixels = _render_obs_view_pixels_impl(state, block_pixel_size, textures_root)
    inv_pixels = _render_inventory_pixels_impl(state, block_pixel_size, textures_root)
    return jnp.concatenate([map_pixels, inv_pixels], axis=0)


@partial(jax.jit, static_argnums=(1,))
def render_craftax_pixels(state, block_pixel_size):
    return _render_craftax_pixels_impl(state, block_pixel_size, TEXTURES)


def make_render_craftax_pixels_jit(texture_theme: str):
    textures_root = get_texture_bundle(texture_theme)

    @partial(jax.jit, static_argnums=(1,))
    def render_for_theme(state, block_pixel_size):
        return _render_craftax_pixels_impl(state, block_pixel_size, textures_root)

    return render_for_theme


def make_render_world_jit(texture_theme: str):
    textures_root = get_texture_bundle(texture_theme)

    @partial(jax.jit, static_argnums=(1,))
    def render_full_base(state, block_pixel_size):
        return _render_full_map_base_impl(state, block_pixel_size, textures_root)

    @partial(jax.jit, static_argnums=(1,))
    def render_obs_overlay(state, block_pixel_size):
        return _render_obs_view_pixels_impl(state, block_pixel_size, textures_root)

    return render_full_base, render_obs_overlay


def render_tile_patch_np(state, x: int, y: int, block_pixel_size: int, texture_theme: str):
    import numpy as np

    textures = get_texture_bundle(texture_theme)[block_pixel_size]
    block_id = int(np.asarray(state.map)[x, y])
    tex = np.asarray(textures["block_textures"][block_id], dtype=np.uint8)
